...
- (void)viewDidLoad {
    ...
    // 创建搜索框
    UIView *titleView = [[UIView alloc] initWithFrame:CGRectMake(20, topHeight, kScreenWidth - 20 - 65, 32)];
    
    UISearchBar *searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(0, 0, titleView.frame.size.width, 32)];
    searchBar.placeholder = @"简约至上";
    searchBar.delegate = self;
    searchBar.barStyle = UIBarMetricsDefaultPrompt;
    ...
    self.navigationItem.rightBarButtonItem = [UIBarButtonItem itemWithTitle:@"取消" selectedTitle:nil target:self action:@selector(cancelDidClick)];
    ...
    titleLabel.textColor = kColor(71, 71, 71);
...
- (void)tagsViewWithTag
{
    ...
    for (int i = 0; i < self.tagsArray.count; i++) {
        ...
        rectangleTagLabel.textColor = kColor(153, 153, 153);
        rectangleTagLabel.backgroundColor = kColor(240, 240, 240);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [closetButton setImage:[UIImage imageNamed:@"shut"] forState:UIControlStateNormal];
