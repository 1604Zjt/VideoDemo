...
//                [self.activityView startAnimating];
        ...
        if (isLogin) {
            ...
            [self.headImageView.imageView sd_setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:[UIImage imageNamed:@"default pic"]];
        }else {
         [self.headImageView.imageView setImage:[UIImage imageNamed:@"default pic"]];
        }
    } else {    // 接收者
        self.retryButton.hidden  = YES;
        self.bubbleView.image    = [UIImage imageNamed:@"liaotianbeijing1"];
        [self.headImageView.imageView setImage:[UIImage imageNamed:self.imageStr]];
    }
}
