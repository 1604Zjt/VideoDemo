...
- (void)viewDidLoad {
    ...
    [self otherSendTextMessageWithContent:[NSString stringWithFormat:@"你可以咨询任何关于%@的问题，专家讲师在线讲解，风格通俗易懂", YTProjectName]];
}

- (void)setupUI
{
    self.view.backgroundColor = IColor(247, 247, 247);
    // 注意添加顺序
...
//    self.tableView.backgroundColor = IColor(240, 237, 237);
    ...
    self.tableView.frame = CGRectMake(0, 0, self.view.width, APP_Frame_Height-HEIGHT_TABBAR-kNavBarHeight-kStatusHeight - kBottomHeight);
}
...
{
    CGFloat bottomHeight = kBottomHeight;
    if (height >= 382) {
        bottomHeight = 0;
    }
    
    self.chatBoxVC.view.top = self.view.bottom - height - kStatusHeight - kNavBarHeight - bottomHeight;
    self.tableView.height = HEIGHT_SCREEN - height - kStatusHeight- kNavBarHeight - bottomHeight;
...
- (void)chatBoxViewController:(ICChatBoxViewController *)chatboxViewController didVideoViewAppeared:(ICVideoView *)videoView
{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        self.tableView.height = HEIGHT_SCREEN - videwViewH;
        self.chatBoxVC.view.frame = CGRectMake(0, videwViewX, App_Frame_Width, videwViewH);
        [self scrollToBottom];
               ...
               sendTextMessage:(NSString *)messageStr
{
    if (messageStr && messageStr.length > 0) {
        ...
        [self otherSendTextMessageWithContent:@"当前老师不在线，请留言"];
...
- (ICChatBoxViewController *) chatBoxVC
{
    if (_chatBoxVC == nil) {
        ...
        [_chatBoxVC.view setFrame:CGRectMake(0, APP_Frame_Height-HEIGHT_TABBAR - kStatusHeight-kNavBarHeight - kBottomHeight , App_Frame_Width, APP_Frame_Height)];
