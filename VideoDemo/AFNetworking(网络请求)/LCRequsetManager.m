...
//  community_demo
//
//  Created by ijianji on 2017/12/11.
...
@implementation LCRequsetManager

// GET请求
+ (void)GET:(NSString *)URLString
 parameters:(id)parameters
responseSeializerType:(LCResponseSeializerType)type
    ...
    if (type != LCResponseSeializerTypeJSON && type != LCResponseSeializerTypeDefault)
    {
...
responseSeializerType:(LCResponseSeializerType)type
    ...
    // 请求头
    [manager.requestSerializer setValue:[self cookie] forHTTPHeaderField:@"application/json"];
    //manager = [[AFHTTPSessionManager manager] initWithBaseURL:[NSURL URLWithString:URLString]];
    // 如果不是JSON 或者 不是Default 才设置解析器类型
    if (type != LCResponseSeializerTypeJSON && type != LCResponseSeializerTypeDefault) {
        manager.responseSerializer = [self responseSearalizerWithSerilizerType:type];
    }
 ...
 */
+ (AFHTTPResponseSerializer *)responseSearalizerWithSerilizerType:(LCResponseSeializerType)serilizerType {
    
    switch (serilizerType) {
            
        case LCResponseSeializerTypeDefault: // default is JSON
            return [AFJSONResponseSerializer serializer];
            break;
            
        case LCResponseSeializerTypeJSON: // JSON
            return [AFJSONResponseSerializer serializer];
            break;
            
        case LCResponseSeializerTypeXML: // XML
            return [AFXMLParserResponseSerializer serializer];
            break;
            
        case LCResponseSeializerTypePlist: // Plist
            return [AFPropertyListResponseSerializer serializer];
            break;
            
        case LCResponseSeializerTypeCompound: // Compound
            return [AFCompoundResponseSerializer serializer];
            break;
            
        case LCResponseSeializerTypeImage: // Image
            return [AFImageResponseSerializer serializer];
            break;
            
        case LCResponseSeializerTypeData: // Data
